...
const Merchantdata = ({ selectedCurrency, coin, showAds }) => {
  ...
  const handlep2pSell = async (e, selectedAd) => {
    ...
    console.log("To sell fiat : ", toSellFiat);
    ...
    const orderDetails = {
      ...
      totalAsset: (toSellFiat / selectedAd.price).toFixed(4),
      paymentDetails: [
        {
  ...
  const validateSellerAssets = async (
    ...
    try {
      const res = await fetch("https://binaryp2p.sytes.net/api/p2p/validateSellerAssets", {
        method: "POST",
        ...
        >
          <div className="border bg-slate-100 text-black mt-16 sm:mt-0 h-full w-full sm:h-3/5 sm:w-[70%] rounded-xl sm:pl-4 flex justify-between items-center">
            ...
            <div className="flex flex-col mt-10 sm:mt-0 gap-6 h-full w-full sm:w-[43%] bg-white sm:px-4">
              ...
              {/* You'll receive */}
              <div className="h-20 mt-3 sm:mt-0 sm:h-24 w-4/5 sm:w-full mx-auto border-slate-300 border rounded-xl px-3 flex flex-col justify-center">
              ...
              <div className="sm:hidden w-full mx-auto fixed bottom-16 border-b border-slate-100 pb-1">
