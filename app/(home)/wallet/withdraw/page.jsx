...
const Withdraw = () => {
  ...
  const handleWithdraw = async () => {
    ...
    if (validWithdrawal) {
      ...
      const res = await fetch("https://binaryp2p.sytes.net/api/wallet/createWithdrawal", {
        method: "POST",
        headers: {
  ...
  const validateBalance = async (amount) => {
    ...
    try {
      const res = await fetch("https://binaryp2p.sytes.net/api/wallet/validateWithdrawal", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
